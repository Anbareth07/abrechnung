...
import type { CostCategory, Property, StromBerechnung, StromPrice, StromReading } from "../api/types";
...
export default function StromPage() {
  ...
  const [readForm, setReadForm] = useState({ reading_date: "", value: "" });
  const [readDel, setReadDel] = useState<StromReading | null>(null);

  const [calcVon, setCalcVon] = useState("");
  const [calcBis, setCalcBis] = useState("");
  const [result, setResult] = useState<StromBerechnung | null>(null);
  ...
  const openReadEdit = (r: StromReading) => {
    setReadEdit(r);
    setReadRole(r.role);
    setReadForm({ reading_date: r.reading_date, value: String(Number(r.value)) });
    setReadOpen(true);
  };
  const saveReading = () => {
    if (!propertyId) return;
  ...
  const runCalc = async () => {
    ...
    try {
      const { data } = await api.get<StromBerechnung>(`/strom/${propertyId}/berechnung`, {
        params: { von: calcVon, bis: calcBis },
      });
      setResult(data);
    } catch {
      err("Berechnung fehlgeschlagen");
    }
  };
  ...
  const canCalc = propertyId != null && calcVon !== "" && calcBis !== "";
    ...
    <Stack>
      ...
      {propertyId != null && (
        <>
          ...
          <Table striped highlightOnHover>
            ...
            <Table.Tbody>
              {KINDS.map((kind) => {
                ...
                if (rows.length === 0) {
                  return (
                    <Table.Tr key={kind.value}>
                      <Table.Td>
                        <Badge variant="light">{KIND_LABEL[kind.value]}</Badge>
                      </Table.Td>
                      <Table.Td colSpan={4}>
                        <Text size="sm" c="dimmed">
          ...
          <Title order={4}>Berechnung</Title>
          ...
          <Group>
            <TextInput type="date" label="Von" value={calcVon} onChange={(e) => setCalcVon(e.currentTarget.value)} />
            <TextInput type="date" label="Bis" value={calcBis} onChange={(e) => setCalcBis(e.currentTarget.value)} />
          ...
          {result && (
            <Stack>
              <Alert color="blue">
                Hauptzähler: {fmt(result.hauptzaehler?.consumption, 2)} kWh · Unterzähler:{fmt(result.unterzaehler?.consumption, 2)} kWh · Netto-Verbrauch:{" "}
                <b>{fmt(result.netto_verbrauch, 2)} kWh</b>
              </Alert>
              <Table striped>
                <Table.Thead>
                  <Table.Tr>
                ...
                <Table.Tbody>
                  {result.positionen.map((p, i) => (
                    <Table.Tr key={i}>
                      <Table.Td>{KIND_LABEL[p.art] ?? p.art}</Table.Td>
                      <Table.Td>
                        {p.von} – {p.bis}
                      </Table.Td>
                      <Table.Td>
                        {p.art === "GRUNDGEBUEHR" ? `${fmt(p.menge, 2)} Jahre` : `${fmt(p.menge, 2)} kWh`}
                      </Table.Td>
                      <Table.Td>
                        {fmtBetrag(Number(p.satz), p.art)} €{p.art === "GRUNDGEBUEHR" ? "/Jahr" : "/kWh"}
                      </Table.Td>
                      <Table.Td>{fmt(p.netto, 2)} €</Table.Td>
                      <Table.Td>{fmt(p.vat, 2)} €</Table.Td>
                      <Table.Td>{fmt(p.brutto, 2)} €</Table.Td>
                    </Table.Tr>
                  ))}
                  <Table.Tr fw={700}>
                    <Table.Td colSpan={4}>Summe</Table.Td>
                    <Table.Td>{fmt(result.summen.netto, 2)} €</Table.Td>
                    <Table.Td>{fmt(result.summen.vat, 2)} €</Table.Td>
                    <Table.Td>{fmt(result.summen.brutto, 2)} €</Table.Td>
                  </Table.Tr>
                </Table.Tbody>
              </Table>
            </Stack>
      ...
      >
        <Stack>
          ...
          />
          <Text size="xs" c="dimmed">
            Startwert ist mit dem letzten bekannten Endwert des Zählers vorbelegt.
          </Text>
          <Group justify="flex-end">
